...
/**
 * This class manages the connection to the Bitcoin network. It connects to the network, listens for
 * new blocks, processes them, and saves the block and transaction data to a PostgreSQL database.
 */
@Service
@Slf4j
public class BitcoinNetworkService {
  ...
  public BitcoinNetworkService(@Value("${bitcoin.targetNode}") String targetNode,
      ...
      BitcoinPeerEventListener bitcoinPeerEventListener) {
  ...
  /**
   * Initializes the service by connecting to the Bitcoin network. This method is called after the
   * bean is constructed.
   */
  @PostConstruct
  public void start() {
  ...
  public CompletableFuture<Void> connectToNetwork() {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        this.peerGroup.addBlocksDownloadedEventListener(
            (peer, block, filteredBlock, blocksLeft) -> blockService.saveBlock(block));
        ...
        log.error("Error connecting to Bitcoin network (attempt {}/{}): {}", attempt, maxRetries,
            e.getMessage());
        if (attempt == maxRetries) {
          throw new RuntimeException(
              "Failed to connect to Bitcoin network after " + maxRetries + " attempts", e);
        }
        try {
  ...
  /**
   * Stops the service and disconnects from the Bitcoin network. This method is called before the
   * bean is destroyed.
   */
  @PreDestroy
  public void stop() {
